...
Connector 保存完成后，为论文父条目填写简明的 `Short Title`，例如 `ResNet`、`BatchNorm`、`DenseNet`、`ViT`。Zotero 列表启用并优先查看“短标题”列。

正式 `Title` 保留论文原始标题，确保引用、检索和 PaperLab 标题匹配不受影响；不修改 PDF 子附件名称。
...
- DOI 或规范化标题与候选记录匹配；
- 可获得的 PDF 位于父条目下面；
- 父条目的 `Short Title` 是清晰且唯一的模型或方法简称；
